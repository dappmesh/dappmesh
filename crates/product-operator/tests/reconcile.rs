...
mod tests {
	...
	async fn should_reconcile_resource_lifecycle() -> Result<(), Error> {
		...
		let operator = ProductOperatorController::new(
		...
		let action = operator.reconcile(Arc::new(product), context.clone()).await?;
		...
		let action = operator.reconcile(Arc::new(product.left_or_default()), context).await?;
